...
def test_mapping_positive(fixture_functional_type_map, fixture_functional_relation_map):
    ...
    # create test object
    test_object = Requirement("DOCUMENT", None, None, "SR001", "Draft", None, None,
                              "The import function shall import a .reqif file and convert it to an .sdoc file",
                              None, None, None, None, special_fields)
    # parse object here
    xml_object = etree.fromstring(object_string_pos, etree.XMLParser())

    # 2 test mapping
    requirement = SpecObjectParser.parse(xml_object, fixture_functional_type_map, fixture_functional_relation_map)

    # 3 assert
    assert (requirement.uid == test_object.uid)
    assert (requirement.status == test_object.status)
    assert (requirement.special_fields[0].field_value == test_object.special_fields[0].field_value)
    assert (requirement.special_fields[1].field_value == test_object.special_fields[1].field_value)
